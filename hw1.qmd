...
Y <- Y_sim(X, 3)
# make df
sim_data=data.frame(X=X,Y=Y) # this is the training data with 100 observations
# ggplot
library(ggplot2)
# create the lines:
...
ggplot(sim_data, aes(x = X, y = Y), color=sim_data) +
  geom_point() +
  geom_line(aes(y = lin_pred, color="Linear")) +
  geom_line(aes(y = quad_pred, color="Quadratic")) +
  geom_line(aes(y = cub_pred, color="Cubic")) +
  geom_function(fun = true_reg, aes(color = "True_regression")) +
  labs(title = "Scatterplot with Fitted Regression Lines",
       x = "X",
       y = "Y")+
  scale_color_manual(values=c('Linear'='blue', 'Quadratic'='red','Cubic'='green','True_regression'='black'))
...
# calculate mse for each model: (note for next time the Metrics package has the mse function)
...
diff<-best_mse-mse_linear
diff

# mse function:
#mse(test_data$Y, test_data$true_pred)
...
run_simulation <- function(n, sigma, test_data) {
  ...
  print(list(results = results, best_model_count = best_model_count))
}
...
# Run the simulation function with sigma = 2
simulation_results <- run_simulation(n = 300, sigma = 4, test_data = test_data)
...
::: {.callout-note title="Solution"}
Simpler models (linear) might have higher bias but lower variance, making them better with more error or less data. This can lead to better performance on test data.

More complex models might have lower bias and so fit the training data better, but have high variance, especially with less data or more error. This can result in bad generalization.
:::
