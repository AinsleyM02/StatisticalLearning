...
print("The loss function used was the default loss function which was the Gini Impurity and the importance was impurity (also Gini). My non-default parameters were num.trees=100 and mtry=3. ")
...
library(yardstick)  # For evaluation functions (roc_curve, roc_auc)
library(ggplot2)    # For plotting
library(dplyr)      # For data manipulation
library(tidyr)      # For pivot_longer

# Prepare a data frame with the predictions from each model
test_perf <- tibble(
  truth = factor(cld_train_s$y, levels = c(1, 0)),  # Ensure binary factor (0 and 1)
  ...
  random_forest = as.numeric(y_hat_rforest)    # Predictions from random forest
)
...
auc_linear <- roc_auc(test_perf, truth, linear)
auc_logistic <- roc_auc(test_perf, truth, logistic)
auc_rf <- roc_auc(test_perf, truth, random_forest)

# Print the AUC values
print(paste("AUC for Linear Model:", auc_linear$.estimate))
print(paste("AUC for Logistic Model:", auc_logistic$.estimate))
print(paste("AUC for Random Forest Model:", auc_rf$.estimate))
```
...
ggplot(roc_combined, aes(x = 1 - specificity, y = sensitivity, color = model)) +
  ...
  labs(title = "ROC Curves for Linear, Logistic, and Random Forest Models",
       ...
       y = "Sensitivity") +
  theme_minimal() +
  scale_color_manual(values = c("Linear" = "blue", "Logistic" = "green", "Random Forest" = "red"))
...
for (i in 1:25) {
  ...
  holdout_idx <- sample(1:nrow(cld_train_s), 500) # holdout size=500
 ...
 #------------------------
  ...
  cv_logreg <- cv.glmnet(X_train, y_train, alpha = 0.75, family = "binomial", nfolds = 10)
  ...
  yhat_logreg <- predict(cv_logreg, newx = X_holdout, s = best_lambda, type = "response")[,1] # adding this one makes it a vector
#---------------------------------------------
  ...
  yhat_rf <- predict(rf_model, data = holdout_data)$predictions 
  ...
  auc_rf[i] <- roc_auc_vec(as.factor(y_holdout), yhat_rf) # make a vector
  
  # make a dataframe/ tible as above and specify which columns from it to pass in below
  Preds_tibble<-tibble(
  truth = factor(y_holdout, levels = c(1,0)),  # Ensure this is binary (0 and 1)
  logreg_preds = as.numeric(yhat_logreg),
  rf_preds=as.numeric(yhat_rf))
  
  # Step vi: Store ROC curve data for plotting, needs df, then columsn specified
  roc_logreg_list[[i]] <- roc_curve(Preds_tibble, truth, logreg_preds)
  roc_rf_list[[i]] <- roc_curve(Preds_tibble, truth, rf_preds)
}
...
```{r}
# Fit the random forest model using ranger on the training data
rf_m <- ranger(y ~ .-y, data = cld_train_s, 
...
# Make predictions on the training set also per the instructions-not typical, usually do it on the test set
preds_rforest <- predict(rf_m, cld_test_s)$predictions
# this has two columsn: prob of no linkage and prob of linkage

# extract the probability of linkage
prob_link_col <- preds_rforest[, 1] 
#prob_link_col

file.remove("McLaughlin_Ainsley_1.csv")
...
# Convert probs to a data frame and explicitly name the column as 'p'
probs_df <- data.frame(p = as.vector(prob_link_col))

# Write to CSV again
write_csv(probs_df, "McLaughlin_Ainsley_1.csv")
```
Click below to get the predictions:

[Download predictions cvs](McLaughlin_Ainsley_1.csv)
...
```{r}
# Fit the random forest model using ranger on the training data
rf_m2 <- ranger(y ~ .-y, data = cld_train_s, 
                     num.trees = 1000, mtry = 5, 
                     importance = 'impurity',
                replace=T, 
                      classification=T,
...
# Make predictions on the training set also per the instructions-not typical, usually do it on the test set
preds_rforest2 <- predict(rf_m2, cld_test_s)$predictions
file.remove("McLaughlin_Ainsley_2.csv")
```

```{r}
# Convert probs to a data frame and explicitly name the column as 'p'
probs_df2 <- data.frame(linkages = as.vector(preds_rforest2))

# Write to CSV again
write_csv(probs_df2, "McLaughlin_Ainsley_2.csv")
