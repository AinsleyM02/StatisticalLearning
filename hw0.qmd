...
-   Networks: `igraph`
-   Forecasting: `fpp3` Ran all of the above
...
$$
\text{MLE} = \frac{d}{d\lambda} \left( L(f(x, \lambda)) \right) = 0 
$$
$$
\frac{d}{d\lambda} \left( \sum_{i=1}^n \log (\lambda e^{-\lambda x_i}) \right) = 0 
$$
$$
\frac{d}{d\lambda} \left( n \log \lambda - \lambda \sum_{i=1}^n x_i \right) = 0 \\
$$
...
::: {.callout-note title="Solution"}
```{r}
# simulate 100 observations from normal 1,1
X<-rnorm(100,1,1)
#X
...
Y <- rnorm(100, mean = 2 * x + 1, sd = sqrt(2))
...
```{r}
# sets initial number of rand generator to 2022
set.seed(2022)
# generates 100 random numbers from a uniform dist with min 2 and max 22
...
::: {.callout-note title="Solution"}
```{r}
#prop.test(successes, total, conf.level = 0.90)
prop.test(95,102, conf.level = 0.90)
# The confidence interval: (0.8720722, 0.9658522). We are 90% confident that the proportion of biopsy images that the machine learning model correctly classifies is between 0.872 and 0.966 on average. 
```
:::
