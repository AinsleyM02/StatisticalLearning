...
print(paste(" The mean lambda min is", mean_cvmin))
mean_1se<-mean(mse_1se)
print(paste("The mean lambda 1 standard error",mean_1se))
...
#Since the p value, which is 1.236e-08, is less than the common confidence level alpha of 0.05, we reject the null hypothesis and conclude that the mse_cvmin and the  mse_1se differ from each other. Since the confidence interval limits are both negative values that means that mse_cvmin-mse_1se<0 which means mse_cvmin<mse_1se. We can conclude that on average the mean mse_cvmin is less than the mean mse_1se. 
```
:::
...
# initialize empty dataframe that will have the alphas and associated rmse and lambda values
...
for (i in seq_along(alphas)){
  ...
  #save rmse and lambda associated with each alpha in the df
  df_a_rmse$rmse[i] <- rmse_val
  df_a_rmse$lambda[i]<-lambda_min
}
# find the min of that vector of rmse's and that associated best alpha is the one you want to use
min_a<-df_a_rmse$alpha[which.min(df_a_rmse$rmse)]
#get the lambda associated with the min rmse
lambda_best_a<-df_a_rmse$lambda[which.min(df_a_rmse$rmse)]
# use just glmnet for the alpha that is best here
fit_glmnet_enet <- glmnet(re_stand_train, re_traindata$price, alpha = min_a)
# the prediction: this does not work:
yhat_enet <- predict(fit_glmnet_enet, newx = re_stand_test, s=lambda_best_a)

#calc min rmse for the print statement:
min_rmse_val<-min(df_a_rmse$rmse)
# print the best alpha and lambda associated with the best rmse value:
print(paste("The alpha associated with the model with the lowest RMSE value(",min_rmse_val, ") is: ",  min_a, " and the minimum lambda at that alpha found by cv.glmnet is ", lambda_best_a))
```
I used the alpha values above which was associated with the lowest rmse of the alpha values list I tried. I then chose the associated lambda value that the cv.glmnet produces using 10 fold cross validation. I used lambda.min as my choice instead of lambda.1se since lambda.min is the minimum lambda and we are choosing the model specifically so we don't necessarily want it to choose a simpler model with the lambda.1se. 
:::
...
::: {.callout-note title="Solution"}
```{r}
#used to remove if necessary when I messed up a bunch
#file.remove("McLaughlin_Ainsley.csv")
```

```{r}
library(readr)

# Convert yhat_enet to a data frame and explicitly name the column as 'yhat'
yhat_enet_df <- data.frame(yhat = as.vector(yhat_enet))

# Write to CSV again
write_csv(yhat_enet_df, "McLaughlin_Ainsley.csv")
```
Click below to get the predictions:

[Download predictions cvs](McLaughlin_Ainsley.csv)

```{r}
# verify this is a dataframe with one column only
#print(dim(yhat_enet_df))
```
:::
...
::: {.callout-note title="Solution"}
```{r}
# Get the min RMSE from the data frame
anticipated_rmse <- min(df_a_rmse$rmse)

# Report the anticipated performance
print(paste("The anticipated RMSE for the elastic net model is:", anticipated_rmse))
```
:::
