...
```{r}
# Restarting based on his notes: 
#library(caret)


# we will put each fold into this knn_eval function

# k fold from cross val notes
knn_eval <- function(kvalues, data_train, data_test){
  ...
  msevalues<-numeric(length(kvalues)) # empty vectors 
  edfvalues<-numeric(length(kvalues))
  # here is the for loop
  for (i in 1:length(kvalues)){
      knn = knn.reg(data_train[,'X', drop=FALSE],# is the training data, predictor
      y = data_train$Y, # outcome
      test = data_train[,'X', drop=FALSE], # testing data-required here
      k = kvalues[i]) # for the ith k
      r_test = data_test$Y - knn$pred # residuals on test data- observed-predicted
      mse_test = mean(r_test^2) # test MSE
      msevalues[[i]]=mse_test
      # results
      edf = nrow(data_train)/kvalues[i] # effective dof (edof)
      #store result
      edfvalues[[i]]=edf
  } # ends the for loop
...
# knn_eval from supervised 2 notes
set.seed(211)
n.folds = 10 # number of folds for cross-validation
fold = sample(rep(1:n.folds, length=nrow(data_sim))) # from 1 to 10, decide the unique folds each will be in


results = vector("list", n.folds) # a list of size ten
#- Iterate over folds
for(j in 1:n.folds){
  ...
  results[[j]] = knn_eval(3:40, # putting in k's being 3 through 40
  data_train = data_sim[train,],
  data_test =data_sim[val,]
  ) %>%
  mutate(fold = j, n.val) # add fold number and number in validation
}
RESULTS = bind_rows(results) #result for every fold for every model
```

```{r}
# plot with $k$ on the x-axis and the estimated MSE on the y-axis
# performance can vary, but around 6,7, and 8
#make the folds a factor
# Plot with k on the x-axis and the estimated MSE on the y-axis

RESULTS$fold <- as.factor(RESULTS$fold)

# Ensure the results object is a proper data frame
RESULTS <- data.frame(RESULTS)

mse_k<-RESULTS%>%group_by(k)%>%summarize(mse=mean(mse_test))
ggplot(mse_k,aes(x = k, y = mse_test)) +
  geom_line() +
  geom_point() +
...
# Simulate your data if not already available (X_fun and Y_fun are functions to generate data)
# Replace this section with your actual data if necessary
...
plot_parta<-ggplot(mse_k, aes(x = k, y = mse)) +
  geom_line(color="blue") +
  geom_point(color="blue")
plot_parta+geom_line(data=test_results, aes(x=k, y=mse_test), color="red")+geom_point(data=test_results, color="red")
```

```{r}
plot_2<-ggplot(data = edf_results, aes(x = edf, y = mse))+geom_point(color="blue")+ geom_line(color="blue")
plot_2+geom_line(data=test_results, aes(x=edf, y=mse_test), color="red")+geom_point(data=test_results, color="red")
...
Unfortunately, I know my plots above are incorrect, but hypothetically here is my logic. 
